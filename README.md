...
skill and ship a changelog entry in the same PR. Because this is a versioned package,
that fix reaches consumers on their next bump. Read [`CONTRIBUTING.md`](CONTRIBUTING.md)
before authoring or heavily editing a skill.

Versioning and releases are managed by
[Changewrite](https://github.com/flipbook-labs/changewrite) (`changewrite.toml` is
the source of truth for the current version, mirrored into `loom.config.luau`).
