...
- **Use `patch` as the catch-all when the tool offers no "skip".** Changewrite requires a bump on every entry and has no internal or skip level, so a change with no user-facing effect (CI, tooling, a pure refactor) takes `patch`. Marin: _"We're essentially using patch as a catch-all for actual patches and anything else a PR could possibly do since there's no skip option."_ Reserve `minor` for surface area a consumer can reach for that did not exist before. Refining how an existing capability behaves takes `patch` even when a consumer notices the change, because how much it registers is not the test. In agent-skills that means `minor` for a new skill and `patch` for an edit to an existing one.
