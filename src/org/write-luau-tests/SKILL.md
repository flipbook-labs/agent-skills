...
The discipline for writing tests worth keeping: what a test is _for_, the red-first gate every test must pass through, the moves that are forbidden, and what "good" looks like. **Scope:** test quality and authoring discipline only. This skill owns _whether a test is any good_, not how a specific repo's test infrastructure works.

## When not to use

For how to _run_ tests in a given repo (the command, filtering, the API keys or services a suite needs), its framework idioms (Jest-Luau matchers, `act()`, fake timers), spec-file placement, and its reference exemplars, see that repo's own test skill (for example a `flipbook/write-tests`). This skill is the doctrine those build on; it names no runner and no framework.
...
**A break-check that stays green has three causes, and only one of them is about the test.** Rule out the other two before you touch the test. The break may never have landed: an edit whose target text did not match leaves the module untouched, so re-read the file and confirm the mutation is actually there. The test may never have run: a filter, a tag, or a conditional wrapping the test can keep it out of the run, so confirm it executed by checking the reported test count or by planting a deliberately failing assertion inside it and watching that fail. Both look identical to a weak test from the outside, and both lead you to rewrite or delete a test that was doing its job. Once the mutation is in the file and the test demonstrably ran, the verdict below applies.

If no deliberate break makes the test fail, the test is decorative. Fix it or delete it; do not leave it in the suite.

**When the gate cannot run** (the suite needs a service, key, or environment you don't have): the gate is _blocked, not waived_. State this explicitly in the PR ("tests written but not seen red/green in this environment") and never imply the gate passed. Whatever static checks _can_ run offline (lint, analyze) still must.

## 3. Forbidden moves

These are the ways agents (and tired humans) game a test suite. All are prohibited:

- **Weakening to green.** A red test is a _finding_, never an obstacle. Do not delete a test, weaken an assertion, broaden a matcher, or add a skip to reach green until you have diagnosed whether the code or the test is wrong, and said which, out loud, in the PR or commit message.
...
- **Orphaned skips.** Do not leave a `skip`ped test in the suite without a reason. A skip is a disabled assertion, so either unskip it, or delete it, or leave a comment saying why it is off and when it comes back. Marin: _"Either unskip or remove this test."_
...
- **The sequence you push through a mock is part of the fixture.** Severing the dependency correctly is half the work. The events you then feed the mock have to be ones the real producer emits, because real sources coalesce repeats, drop identical values, and throttle. Feed a mock a sequence reality never produces and the test locks in behavior for a case that cannot happen, while the path that does run stays uncovered. Read how the real producer behaves, then match it.
...
- **An assertion of absence needs a positive control in the same test.** "Nothing happened" passes exactly as well when the stimulus never arrived, so a test that only asserts a thing did not occur cannot tell a working guard from an input that vanished on the way in. Assert alongside it that the stimulus had some other observable effect: a value it does legitimately change, a sibling callback that still fires, a state it still reaches. That second assertion is what keeps the test honest when the wiring it depends on breaks.
...
- **Assert across every analogous case, not just one.** When a family of constructors, factories, or branches share an invariant, assert it in all of them, not in a representative one. Marin, on a set of control constructors: _"If we're going to assert here, we should also assert in the other constructors where necessary,"_ especially that the mappings actually map correctly. A test that checks one member of a family lets the others regress silently.
- **Choose the assertion and the fixture values that separate correct from incorrect.** A concrete-looking assertion can still hold under the very bug you are guarding against, and an inequality is the usual offender: `expect(value).toBeLessThan(previous)` is satisfied whether the value stepped down by one increment or collapsed to the middle of its range. When the behavior is deterministic, assert the exact expected value. Then check the fixture, because a starting value that makes the correct and incorrect results arithmetically identical hides the failure no matter how precise the matcher. A slider that starts at the midpoint of its range cannot distinguish a reversal that continues from where it sat from one that re-anchors to the center, so start it somewhere else.
...
- [ ] Any break-check that stayed green was triaged before the test took the blame: the mutation is confirmed present in the file, and the test is confirmed to have run (§2).
- [ ] Any test asserting that something does _not_ happen also asserts that the stimulus had an observable effect (§4).
...
- [ ] If a pre-existing test was modified or deleted, the PR explains why the _test_ was wrong, not only that it now passes.
...
Updated 2026-07-24 with break-check triage, positive controls for absence assertions, mock event sequences as fixture, and discriminating assertions and fixture values. All four come from one session adding directional-input coverage to a Roblox slider component, where each rule maps to a specific false result the existing doctrine would have misread. A break-check stayed green because the edit that was supposed to break the module never matched its target, which §2 would have scored as a decorative test. A hook test drove a mocked input signal with repeated identical values that the real input layer coalesces, so it passed for a case production cannot reach while the reachable case went uncovered. Continuity assertions written as inequalities held under both the fixed and the broken implementation, and starting the slider at the midpoint of its range made a reset-to-center bug arithmetically indistinguishable from correct behavior.

**Re-verify these claims when this skill next loads:** this skill is pure doctrine and makes no claims about any repo's source, so it has no volatile layer to re-derive. When a consuming repo's test skill drifts from this doctrine (a forbidden move creeps into its exemplars, or its red-first wording weakens), fix it there, and if the _principle_ changed, update this skill and add a `.changes/` entry.

**Cross-model eval (2026-07-05):** a Sonnet model, guided only by this skill, wrote tests for a function with a seeded boundary bug. It tested the _intended contract_ rather than the current behavior, correctly predicted the contract tests would go red, and flagged the bug to the author rather than transcribing it, landing in the acceptance band with no refinement needed. Re-run this kind of check (a weaker or different model, a task with a latent bug, does the skill steer it to the contract?) when materially editing §1 through §3.
