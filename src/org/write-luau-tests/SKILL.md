...
**A break-check that stays green has three causes, and only one is about the test.** Rule out the other two first, because both look identical to a decorative test from the outside. The break may never have landed: an edit whose target text did not match leaves the module untouched, so re-read the file and confirm the mutation is there. The test may never have run: a filter, a tag, or a conditional wrapping the test can keep it out of the run, so check the reported test count, or plant a deliberately failing assertion inside the test and watch that fail. Only once the mutation is in the file and the test demonstrably ran does the verdict below apply.
...
- **Choose the assertion and the fixture values that separate correct from incorrect.** A concrete-looking assertion can still hold under the very bug you are guarding against, and an inequality is the usual offender: asserting a value is _less than_ the previous one holds whether it stepped down by a single increment or collapsed to the middle of its range. When the behavior is deterministic, assert the exact expected value. Then check the fixture, because a starting value that makes the correct and incorrect results arithmetically identical hides the failure no matter how precise the assertion. Start a slider at the midpoint of its range and a reversal that continues from where it sat produces the same value as one that re-anchors to the center, so start it somewhere else.
