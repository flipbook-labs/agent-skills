...
For how to *run* tests in a given repo (the command, filtering, the API keys or services a suite needs), its framework idioms (Jest-Luau matchers, `act()`, fake timers), spec-file placement, and its reference exemplars, see that repo's own test skill (e.g. a `flipbook/write-tests`). This skill is the doctrine those build on; it deliberately names no runner and no framework.
...
**When the gate cannot run** (the suite needs a service, key, or environment you don't have): the gate is *blocked, not waived*. State this explicitly in the PR ("tests written but not seen red/green in this environment") — never imply the gate passed. Whatever static checks *can* run offline (lint, analyze) still must.
...
- **Names are behavioral specs.** `"runs the callback immediately on the first call"`, `"throws when the id is unknown"` — a reviewer should reconstruct the contract from the test names alone. Not `"test callback"`, not `"works correctly"`.
- **Test the public contract, not internals.** Assert what a caller observes — return values, thrown errors, callback invocations, emitted values, observable side effects — never private state, internal call order, or intermediate representations.
...
For code whose job is a side effect rather than a return value (UI that renders, an event that fires, a store that notifies), the guiding principle is: **the more a test resembles how the code is actually used, the more it proves.** Drive the unit the way its real caller does (fire the event, apply the input, advance the clock), then assert on what an observer downstream would see — not on internal bookkeeping. Assert cleanup too when the unit owns a resource. Framework-specific mechanics for doing this live in the relevant repo's test skill.

## 6. Pre-done checklist

Before declaring test work complete, confirm every line:

- [ ] Every new/changed test has been seen **red** at least once — via test-first failure or a deliberate break (§2). If the gate was blocked, the PR says so explicitly.
...
**Date stamped:** 2026-07-05. Doctrine extracted and generalized from Flipbook's `write-flipbook-tests` skill; this is the vendor-neutral core, with runner/framework/exemplar specifics left to each repo's own test skill.

**Re-verify these claims when this skill next loads:** this skill is pure doctrine — it makes no claims about any repo's source, so it has no volatile layer to re-derive. When a consuming repo's test skill drifts from this doctrine (a forbidden move creeps into its exemplars, or its red-first wording weakens), fix it there and, if the *principle* changed, update this skill and ship a `CHANGELOG.md` entry.

**Cross-model eval (2026-07-05):** a Sonnet model, guided only by this skill, wrote tests for a function with a seeded boundary bug. It tested the *intended contract* (not the current behavior), correctly predicted the contract tests would go red, and flagged the bug to the author rather than transcribing it — landing in the acceptance band with no refinement needed. Re-run this kind of check (weaker/different model, task with a latent bug, does the skill steer it to the contract?) when materially editing §1–§3.
