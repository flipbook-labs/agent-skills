<!-- doctrine-exempt rules="em-dash" reason="The default draft this example criticizes, where the dashes are the same carelessness as the derivable prose around them." -->
...
Both are filed at `org/` scope with frozen inline exemplars and no single-repo anchors — the patterns should hold for any flipbook-labs repo that grows a react-lua UI. The AGENTS.md index and a `.changes/` entry (minor bump) land in the same commit per CONTRIBUTING.md.
