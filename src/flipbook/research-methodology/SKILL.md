...
- **Solution:** `.lute/test.luau` supports `--build-only` and `--run-only`, keeping place construction separate from Rocale execution while preserving `--filter <pattern>` locally.
...
**The invariant:** Ideas flow through change-control gates (change-control skill; CI validation via validation-and-qa skill). Never route around them. Routing around gates is how bugs ship.

### Gate 1: CI / Automated Validation (validation-and-qa)
...
### Gate 2: Change-Control Classification (change-control)
...
### Gate 3: Testing & Measurement (diagnostics-and-tooling + validation-and-qa)
...
### Gate 4: Community Doctrine (community-and-positioning)
...
**Lesson:** When a branch reaches >1 week stalled, capture a verdict in failure-archaeology, even if tentative:
...
10. **Merge as PR** with AI authorship disclosed (from change-control): "Generated with Claude Code."
...
- **Stalled branches:** Run `git branch -a | wc -l` every quarter. Branches >4 weeks stalled should have a verdict in failure-archaeology or be merged.
