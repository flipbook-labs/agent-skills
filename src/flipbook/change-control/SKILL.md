...
**Review routing:** `.github/MERGE_POLICY.md` is the policy source of truth, and the [Flipbook Labs GitHub governance stack](https://github.com/flipbook-labs/infra/tree/main/stacks/github) manages the live ownership and enforcement configuration. Ownership is inferred from changed files. Application paths under `workspace/flipbook-core` and `workspace/flipbook-next` belong to `@flipbook-labs/flipbook-app`, except workspace `rotriever.toml` release manifests, which belong to `@flipbook-labs/flipbook-engine`. Every other path, including repository stewardship, also belongs to engine. Application changes require one human application-team approval. While the engine team has one member, engine changes require no human approval but still require the current Greptile 5/5 status and all applicable CI; increase the engine minimum to one approval when a second member joins. Mixed changes follow both paths.
...
4. **Major version bumps** to Storyteller or ModuleLoader are high-risk.
   ...
   - Use the `test-dependencies` skill to overlay local builds into Flipbook and validate in isolation before landing.
...
**Incident (Path-Length Saga, #518–#530):** When MAX_PATH broke Windows builds (260-char limit), the fix was PR #523 "Bundle packages as rbxms" (packToRbxm call in `.lute/lib/build-system/compileAsync.luau`, verified 2026-07-01). PR #530 removed the detection logic once bundling worked. The permanent rbxm bundling lives in the build script, not in the output. Editing `build/` directly would lose this on the next rebuild.
...
**Incident (AGENTS.md section "Comments explain the present, not the history"):** Formalizes the rule that comments must stand on their own; git history is the source for "how we got here."
...
**See also:** AGENTS.md (repo root, section "Comments explain the present, not the history"; grep that phrase) for the full doctrine.
...
**Last verified:** 2026-09-13 (against `.github/MERGE_POLICY.md`, the pull request template, Changewrite v0.7.0, repository workflows, and repo files).
...
# Confirm test-dependencies skill exists
ls ~/.loom/store/AgentSkills@*/src/flipbook/test-dependencies/SKILL.md
```
