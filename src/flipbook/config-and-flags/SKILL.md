...
**When NOT to use:** for build pipeline mechanics, use `build-and-toolchain`; for telemetry strategy specifics, use `config-and-flags` (sections 5 and A); for release workflows, use `release-and-operations`.
...
| --------------------------------- | ------------------------------------------------------------- | ------------------------------- | -------------------------------------------------- | -------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LOG_LEVEL`                       | Min verbosity for logs sent to Help > Logs                    | `info`                          | `trace`, `debug`, `info`, `warn`, `error`, `fatal` | Production           | `workspace/flipbook-core/src/logger.luau` sets `MinLevelFilter(_G.LOG_LEVEL)`                                                                                                |
| `ENABLE_OUTPUT_LOGGING`           | Route logs to Studio Output window (in addition to Logs view) | `false`                         | `true` or `false`                                  | Production           | `workspace/flipbook-core/src/logger.luau` checks `_G.ENABLE_OUTPUT_LOGGING == "true"`                                                                                        |
| `BASE_URL`                        | Telemetry and feedback backend endpoint                       | `https://apis.flipbooklabs.com` | Valid HTTPS URL                                    | Production           | `workspace/flipbook-core/src/Telemetry/fireEventAsync.luau`, `workspace/flipbook-core/src/Feedback/postFeedbackAsync.luau` build both requests to `{_G.BASE_URL}/<endpoint>` |
| `ROBLOX_API_KEY`                  | Open Cloud API key for cloud Jest test execution              | (required, no default)          | Valid Open Cloud key                               | Production           | `.lute/build.luau` calls `dotenv.config()`; used by test runner only                                                                                                         |
| `ROBLOX_UNIT_TESTING_PLACE_ID`    | Test place for cloud Jest execution                           | `123506190725771`               | Roblox place ID                                    | Production           | `.env.template` (grep `ROBLOX_UNIT_TESTING_PLACE_ID=`) and build system                                                                                                      |
| `ROBLOX_UNIT_TESTING_UNIVERSE_ID` | Test universe for cloud Jest execution                        | `6599100156`                    | Roblox universe ID                                 | Production           | `.env.template` (grep `ROBLOX_UNIT_TESTING_UNIVERSE_ID=`) and build system                                                                                                   |
...
| ----------- | ------------------------------------------------------ | ---------------------------------- | ------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------- |
| `roblox`    | **Yes** (`.lute/build.luau` grep `default = "roblox"`) | Roblox Studio plugin               | `build/<channel>/roblox/` → `Flipbook.rbxm`                              | Standard plugin; no internal APIs                                                                                                          | Community releases                                   |
| `rotriever` | No                                                     | Roblox internal Rotriever registry | `build/<channel>/rotriever/` → `flipbook-core-rotriever/` package bundle | Uses internal Roblox APIs (`_G.INTERNAL_BUILD_TARGET == BuildTarget.Rotriever` badge shown in `FlipbookApp.luau` grep `showInternalBadge`) | Internal Roblox builds; not shipped to Creator Store |
...
| --------------------------- | --------- | -------- | ---------- | ------------------------------------------------------------------------------------- | ------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `rememberLastOpenedStory`   | Stories   | boolean  | `true`     | `true`\|`false`                                                                       | Restore the last-viewed story on plugin open                              | `workspace/flipbook-core/src/Storybook/useLastOpenedStory.luau` (grep `rememberLastOpenedStory`) checks setting and skips restore if false               |
| `theme`                     | UI        | dropdown | `"system"` | `"system"`, `"dark"`, `"light"`                                                       | UI theme: match Studio, force dark, or force light                        | `workspace/flipbook-core/src/Storybook/StoryViewNavbar.luau` (grep `userSettings.theme`) passes setting as current dropdown value                        |
| `sidebarWidth`              | UI        | number   | `260` px   | `140`–`500` px (per `constants.luau` grep `SIDEBAR_MIN_WIDTH\|SIDEBAR_MAX_WIDTH`)     | Sidebar panel width, persisted across sessions                            | `workspace/flipbook-core/src/FlipbookApp.luau` (grep `userSettings.sidebarWidth`) sets `initialSize` UDim2                                               |
| `controlsHeight`            | UI        | number   | `200` px   | `100`–`400` px (per `constants.luau` grep `CONTROLS_MIN_HEIGHT\|CONTROLS_MAX_HEIGHT`) | Story controls panel height, persisted                                    | `workspace/flipbook-core/src/StoryControls/StoryControlsPanel.luau` (grep `userSettings.controlsHeight`) sets panel `initialSize` UDim2                  |
| `collectAnonymousUsageData` | Telemetry | boolean  | `true`     | `true`\|`false`                                                                       | Opt in to anonymous usage metrics; user shown opt-in dialog on first open | `workspace/flipbook-core/src/Telemetry/fireEventAsync.luau` (grep `collectAnonymousUsageData`) returns early if false; prevents all telemetry collection |
...
| Name                           | Value                                                                                      | Purpose                                                                                                       |
| ------------------------------ | ------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------- |
| `PROD_CONFIG.prunedDirs`       | `{ workspace/code-samples, workspace/example, workspace/template, workspace/test-runner }` | Directories stripped from prod builds via Darklua dead-code elimination (grep `prunedDirs` in `project.luau`) |
| `PROD_CONFIG.prunedFiles`      | `{ *.spec.lua*, *.story.lua*, *.storybook.lua*, jest.config.lua* }`                        | File patterns stripped from prod builds (grep `prunedFiles` in `project.luau`)                                |
...
**Example:** `FlipbookApp.luau` (grep `showDevBadge\|showBetaBadge\|showInternalBadge`) shows dev/beta/internal badges only in non-prod builds.
...
1. **JEST_TEST_PATH_PATTERN drift:** Passed via `--filter` to build, but not in `.env.template`. Verify it's documented in test runbook (see `validation-and-qa` skill).
