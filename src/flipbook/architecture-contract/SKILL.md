...
**Location:** `src/PluginStarterScript.plugin.luau` (comment block above `Charm.flags.frozen = false`; grep `Normally we want this to be`)
...
**Location:** `workspace/flipbook-core/src/TreeView/createTreeNodeStore.luau` (setSortOrder function; grep `function setSortOrder` or the FIXME cast comment)
...
**Location:** `workspace/flipbook-core/src/StoryControls/ControlElements/CheckControl.luau` (grep `-- TODO: Make this a grid`)
...
**Location:** `workspace/flipbook-core/src/Storybook/types.luau` (ExtraStoryProps type definition; grep `TODO: Make these reequired`)
...
**Location:** `workspace/flipbook-core/src/createFlipbookApp.luau` (grep `FIXME: ContextProviders having an error`)
...
**Location:** `workspace/flipbook-core/src/stories.spec.luau` (grep `FIXME: This is needed to get around a bug with React renders`)
...
**Status:** uilabs-controls-support branch stalled (May 9 2026). See the story-controls-campaign skill for the current plan and ranked options.
...
**Related skills:** build-and-toolchain, domain-reference, story-controls-campaign, debugging-playbook
