...
**When NOT to use this skill:** for local development builds (→ build-and-toolchain); for debugging Luau code or tests (→ debugging-playbook); for architecture decisions (→ architecture-contract); for telemetry/user-facing settings (→ config-and-flags).
...
**Key constraint (maintainer doctrine; the `change-control` skill is the in-repo home for it):** Never push git tags directly. Releases go through `bump-version` PR → merge → manual GitHub Release creation. The rationale is the deployment-orchestration history in `failure-archaeology` (incidents #535, #596): direct tag pushes bypass the changelog/version gating and can fire deploy workflows unexpectedly.
...
| ---------------------- | ------------------ | ------------------------ | -------------------------------- |
| publish-github-release | release event only | none                     | Attaches .rbxm to release assets |
