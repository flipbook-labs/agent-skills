...
description: "Instrumentation and observability: how to measure Flipbook's runtime behavior. Use when: debugging why a story doesn't load (add logging), validating a performance fix (measure rerenders), verifying build inclusion (inspect sourcemaps/cache), or filtering tests. Covers logging system, test output parsing, build-cache inspection, sourcemap require-path tracing, and rerender accounting. Complements validation-and-qa (evidence standards for fixes) by providing the measurement tools."
type: process
---
...
The logger has two sinks, both instantiated in `logger.luau` (in the `sinks` table initialization):
...
2. **Roblox Output Window (opt-in)**: Only added when `_G.ENABLE_OUTPUT_LOGGING == "true"` (grep `if _G.ENABLE_OUTPUT_LOGGING == "true"`). This lets you see logs in Studio's Output tab for easier copy-paste and search.
...
The `MinLevelFilter` (grep `MinLevelFilter`) gates events below the configured level; only matching events reach the sinks.
...
The `--filter` argument is injected into `JEST_TEST_PATH_PATTERN` env var by `.lute/test.luau` (grep `process.env.JEST_TEST_PATH_PATTERN`), which is then injected by Darklua into `_G.JEST_TEST_PATH_PATTERN` at build time. The test runner (in `workspace/test-runner/src/init.luau`) reads this global and passes it to Jest as `testPathPattern = _G.JEST_TEST_PATH_PATTERN`.
...
1. `.lute/test.luau` builds a dev plugin and packs test place into a temporary rbxl file (grep `rojoBuild` call with `output = path.join`)
2. Rocale invokes `.lute/tasks/run-tests.luau` script in that cloud place, passing API key and place IDs (grep `run("rocale"` in `.lute/test.luau`)
3. `run-tests.luau` requires the test runner from ReplicatedStorage and calls `TestRunner.runTests()` (in `.lute/tasks/run-tests.luau`, grep `TestRunner.runTests()`)
