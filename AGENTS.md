...
- `org/durable-writing`: write PR bodies, commit bodies, comments, and changelog entries for the future reader who has the artifact but not your session. The transition-versus-destination root, the derivability and present-moment tests, the session-salience tell (is this true about the code or about how I got here), and reframing transient context durably. The cross-cutting root that `org/code-comments` and `org/changelog-entries` each cover one slice of. Read before writing any prose attached to a change. `type: process`.
