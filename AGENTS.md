...
- `org/durable-writing`: write PR bodies, commit bodies, comments, and changelog entries for the future reader who has the artifact but not your session. The transition-versus-destination root, the derivability and present-moment tests, the session-salience tell (is this true about the code or about how I got here), reframing transient context durably, and choosing tense from the destination frame (present tense for a section that describes the artifact, past tense only for real completed events). The cross-cutting root that `org/code-comments` and `org/changelog-entries` each cover one slice of. Read before writing any prose attached to a change. `type: process`.
...
- `org/visual-evidence`: capture, verify, and attach screenshots or short videos for user-visible changes, including matched before/after footage when real prior behavior is reproducible. Use when: a reviewer needs visual proof, an interaction needs recording, or change media belongs in a PR. `type: process`.
...
- `flipbook/use-studio-mcp-for-flipbook`: build and validate Flipbook through Studio MCP and FlipbookAgentGateway. Use when: driving a local Flipbook build, changing gateway actions, or verifying Stories through semantic actions, embedded Play-mode input, and screenshots. `type: process`.
...
- `flipbook/validation-and-qa`: evidence bar for proving a fix, test anatomy, visual capture, and QA standards for Flipbook. Use when: writing tests, proving a fix is correct, capturing screenshots or video, or understanding what evidence is required. `type: process`.
